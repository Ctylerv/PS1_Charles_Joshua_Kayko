...
```
The estimates for $\beta_1$ and $\beta_2$ are not biased, as the estimates are normally distributed about their specified values (as can be seen from the method of data generation above in the group exercise description). 
...
```
Yes, $\beta_1$ is biased because we have omitted the variable $V_2$ from this regression. 
...
ps1_reg2 <- lm(adjY ~ V1, out)
summary(ps1_reg2)

```
This estimated value of $\beta_1$, which is the same as what we found for the first regression, is not biased. This is because we have accounted for the effect of $V_2$ in our new dependent variable $adjY$. 
...
The original plot is above. Our improved plot is below: 

```{r improved plot}

# You'll need to install the ggpubr package before running this code. 
library(ggpubr)

p1 <- ggplot(out, aes(V1, y = value)) + 
  geom_point(aes(y = Y, col = "Y"), alpha=0.4) + 
  geom_point(aes(y = adjY, col = "adjY"), alpha=0.4)+ 
  geom_smooth(method='lm', aes(y = Y, col = "Y"), color = "#00205B") +
  geom_smooth(method='lm', aes(y = adjY, col = "adjY"), color = "#B58500") +
...
p2 <- ggplot(out, aes(x=Y)) + 
	...
	scale_x_continuous(limits = c(-35, 35), position = "top") + 
	scale_y_continuous(limits = c(0, 0.125)) +
	coord_flip() +
	theme_void() +
	theme(axis.text.y = element_blank(), axis.text.x = element_text(color = "white", size = 20))
p3 <- ggplot(out, aes(x=adjY)) + 
	...
	theme_void() +
	theme(axis.text.y = element_blank(), axis.text.x = element_text(color = "white", size = 20))
improved <- ggarrange(p1, p2, p3, widths=c(4, 0.5, 0.5), heights=c(4, 0.5, 0.5), ncol=3)
