...
```{r question1}
ps1_reg1 <- lm(data=out, Y ~ V1+V2)
summary(ps1_reg1)

beta1 <- coef(ps1_reg1)[2]
beta2 <- coef(ps1_reg1)[3]
...
```{r question2}
ps1_reg2 <- lm(data=out, Y ~ V1)
summary(ps1_reg2)

beta1_OVB <- coef(ps1_reg1)[2]
beta1_OVB

```
The estimate for $\beta_1$ is biased because we have omitted the variable $V_2$ from this regression.  
...
out$adjY <- out$Y - beta2*out$V2

ps1_reg2 <- lm(data=out, adjY ~ V1)
summary(ps1_reg2)

```
This value of $\beta_1$ is not biased, because we have accounted for the effect of $V_2$ in our new dependent variable $adjY$ in this regression. 
...
p1 <- ggplot(out, aes(V1, y = value)) + 
    geom_point(aes(y = Y, col = "Y"), color="#003594", alpha=0.4) + 
    geom_point(aes(y = adjY, col = "adjY"), color="#FFB81C", alpha=0.4)+
    geom_smooth(method='lm', aes(y = Y, col = "Y"), color="#00205B")+
    geom_smooth(method='lm', aes(y = adjY, col = "Y"), color="#B58500") +
...
p3 <- ggplot(out, aes(x=adjY)) + 
	...
	coord_flip() +
	theme_void()
ggarrange(p1, p2, p3, widths=c(3, 0.5, 0.5), ncol=3)
```
